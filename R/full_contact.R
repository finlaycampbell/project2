...
result.analysis <- function(result,true.outbreak,plot=TRUE,print=FALSE){
  ...
  square.conf <- round(mean(sapply(result[transmission.id+adder],function(i) sum(table(i)^2)/samples^2)),2)
  
  mean.conf <- round(mean(sapply(transmission.id,function(i) mean(result[[i+adder]]==true.outbreak$ances[i]))),2)
...
compare.outbreakers <- function(runs=10,min.hosts=10,max.hosts=15,eps=0.8,xi=0.01){
  ...
  while(counter<=runs){
    true.outbreak <- outbreaker::simOutbreak(2,temp.w.dens,n.hosts=max.hosts,mu.transi=1e-05,rate.import.case=0)
    if(true.outbreak$n<min.hosts) next
    
    print(counter)
    
    sim.contact <- simCTD(true.outbreak,plot=TRUE,eps=0.8,xi=0.01)
    
    temp.time <- system.time(their.result <- CTD.outbreaker(data=list(dates=true.outbreak$onset,w.dens=temp.w.dens,
                                             dna=true.outbreak$dna),config=list(n.iter=2e4, sample.every=200)))
    analysis <- result.analysis(their.result,true.outbreak,print=FALSE,plot=FALSE)
    ...
    analysis <- result.analysis(our.result,true.outbreak,print=FALSE,plot=FALSE)
    out[counter,c(2,4,6,8)] <- c(analysis$prop.correct,analysis$entropy,analysis$mean.conf,temp.time[1])
    
    counter <- counter + 1
    
    print(paste("~",round(mean(c(out[,7],out[,8]),na.rm=TRUE)*(runs-counter)*2/60,0)," minutes left",sep=""))
  }
  ...
  p <- ggplot(plot.out) + geom_violin(aes(x=variable,y=value,fill=Model),scale="width") + ylim(0.,1) +
       ggtitle(paste(eps*100,"% coverage | ",xi," false positive rate",sep="")) + ylab("Value") +
       theme_set(theme_gray(base_size = 18)) + theme(axis.title.x=element_blank())
  
  print(p)
  
  ggsave("C:\\Users\\Finlay\\Documents\\Projects\\project2\\figs\\compare.outbreaker.png",p)
...
compare.lineplot <- function(){
  ...
  for(eps in seq(0.1,1,0.1)){
    load(paste("C:\\Users\\Finlay\\Documents\\Projects\\project2\\output\\eps0",eps*10,"ksi001.RData",sep=""))
    temp <- sapply(analysis$analysis,mean)
    
    absolute.analysis[eps*10,1:8] <- temp
    full.abs.analysis[(1:runs)+runs*(eps*10-1),1:8] <- as.matrix(analysis$analysis)
    
    for(i in 1:4) relative.analysis[eps*10,i] <- (temp[2+2*(i-1)]/temp[1+2*(i-1)]) - 1
    relative.analysis[eps*10,5] <- absolute.analysis[eps*10,9] <- eps
    full.abs.analysis[(1:runs)+runs*(eps*10-1),9] <- eps
  }
  
  colnames(relative.analysis) <- c("Accuracy","Entropy","Confidence","Time","Eps")
  ggplot.rel.analysis <- melt(as.data.frame(relative.analysis),id="Eps")
  names(ggplot.rel.analysis) <- c("Eps","Variable","Value")
  ...
  colnames(absolute.analysis) <- c(names(temp),"Eps")
  ggplot.abs.analysis <- melt(as.data.frame(absolute.analysis),id="Eps")
  ...
  names(ggplot.abs.analysis) <- c("Eps","Variable","Value","Model")
  
  colnames(full.abs.analysis) <- c(names(temp),"Eps")
  ggplot.full.abs.analysis <- melt(as.data.frame(full.abs.analysis),id="Eps")
  ...
  names(ggplot.full.abs.analysis) <- c("Eps","Variable","Value","Model")
  
  
  p <- ggplot(ggplot.rel.analysis,aes(x=Eps,y=Value,col=Variable)) + geom_line(size=1.5) + 
       xlab("Contact tracing coverage") + ylab("Relative change to Outbreaker (%)")
  p
  
  q <- ggplot(ggplot.abs.analysis,aes(x=Eps,y=Value,col=Model)) + geom_line(size=1.5) + 
       facet_grid(Variable ~ .,scales="free") + xlab("Contact tracing coverage")
  q
  
  r <- ggplot(ggplot.full.abs.analysis,aes(x=Eps,y=Value,col=Model)) + stat_smooth() +
    facet_grid(Variable ~ .,scales="free") + xlab("Contact tracing coverage")
  r
  ...
  return(list("rel.plot"=p,"abs.plot"=q))
}